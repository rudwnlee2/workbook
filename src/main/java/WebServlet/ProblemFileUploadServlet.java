...
@WebServlet("/problemUpload")
@MultipartConfig
public class ProblemFileUploadServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        DB.loadConnect();
        try {
            DB.ProblemTransaction(membershipNumber, title, content, category, correctAnswer, solutionProcess, fileName, filePath);
            DB.updateMemberLevel(membershipNumber); // 등급 업데이트 로직 추가
            response.getWriter().println("<script>alert('문제가 성공적으로 작성되었습니다.'); location.replace('ProblemList.jsp');</script>");
        } catch (SQLException e) {
            e.printStackTrace();
            response.getWriter().println("<script>alert('문제 전송 실패. 관리자에게 문의하세요.'); location.replace('content.jsp');</script>");
        } finally {
            DB.disconnect();
        }
    }
